...
- `storefront-web/app/page.tsx` 从目录读取首页章节商品。`components/landing/LandingPage.tsx` 提供桌面米白首页及本地导航和页尾；`app/landing.css` 限定首页样式，`components/landing/motion/useUnpackMotion.ts` 用一条固定画面的滚动时间线打开背包，并依次演出饮水、睡眠、照明和炊具四个章节，每章包含多个透明商品变体和聚焦信息。`public/images/landing/` 保存主视觉与章节透明素材，来源与提示词见 `data/landing-image-prompts.json` 与 `data/unpack-image-prompts.json`。
...
- `public/images/` 保存历史场景素材与首页通用图片；当前首页只使用 `public/images/landing/` 的透明背包和装备精灵。`data/scene-image-prompts.json` 记录保留的场景来源。`public/fonts/` 保存自托管中文标题字体及开放字体许可证，服务器构建无需下载字体。
