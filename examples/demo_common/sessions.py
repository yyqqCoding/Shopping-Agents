...
``start`` is the only place a principal enters the store: it binds a shopper profile to
a fresh unguessable session id. Every later request carries that id in
``SESSION_HEADER`` and the routes read the principal from the
